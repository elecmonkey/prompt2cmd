...
func (p *Provider) AuditExecutionResult(command string, result string, prompt string) (*llm.ExecutionAuditResult, error) {
	// 处理结果为空的情况
...
- 无输出不一定意味着失败，某些命令执行成功后可能没有输出`
	...
	// 解析JSON响应
	var auditResult llm.ExecutionAuditResult
